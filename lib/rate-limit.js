...
module.exports = function createRateLimitChecker(CORSANYWHERE_RATELIMIT) {
  ...
  let rateLimitConfig = /^(\d+) (\d+)(?:\s*$|\s+(.+)$)/.exec(CORSANYWHERE_RATELIMIT);
  if (!rateLimitConfig) {
    // No rate limit by default.
    return function checkRateLimit() {};
  }
  let maxRequestsPerPeriod = parseInt(rateLimitConfig[1]);
  let periodInMinutes = parseInt(rateLimitConfig[2]);
  let unlimitedPattern = rateLimitConfig[3]; // Will become a RegExp or void.
  if (unlimitedPattern) {
    let unlimitedPatternParts = [];
    unlimitedPattern.trim().split(/\s+/).forEach(function(unlimitedHost, i) {
      let startsWithSlash = unlimitedHost.charAt(0) === '/';
      let endsWithSlash = unlimitedHost.slice(-1) === '/';
